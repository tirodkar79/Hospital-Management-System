...
def doctor():
    ...
    try:
        cur.execute("select*from instru")
        result = cur.fetchall()
        count = cur.rowcount
    ...
    home.title("Instrument")

    def go_back():
        home.destroy()

    head_label = Label(home, text="Instrument", background="turquoise1", foreground="white",
    ...
    id_label = Label(home,text="Instrument Id",bg="snow2",fg = "black",font=('Times', 16, 'bold'),relief = "solid",padx=5,width=13).place(x=258,y=130)

    id_Name = Label(home, text="Instrument Name", bg="snow2", fg="black", font=('Times', 16, 'bold'), relief="solid", padx=5,
                     width=27).place(x=425, y=130)
    id_quali = Label(home, text="In Stock", bg="snow2", fg="black", font=('Times', 16, 'bold'), relief="solid", padx=5,
                     width=10).place(x= 761, y=130)
    id_specality = Label(home, text="Recent Renewal date", bg="snow2", fg="black", font=('Times', 16, 'bold'), relief="solid", padx=5,
                     width=17).place(x=893, y=130)
    global z
    z = 159
    global flag
    flag =0
    ...
    def add():
        ...
        def done():
            ...
            if (pharma_name != "" and pharma_quantity != "" and pharma_recent != ""):
                ...
                try:
                    ...
                    pharma_id = count + 1
                    sql = "insert into instru(instru_id, instru_name,quantity,r_date) values(%s,%s,%s,%s)"

                    val = (pharma_id, pharma_name, pharma_quantity,pharma_recent)
                    cur.execute(sql, val)
        ...
        def check():
            ...
            if (flag == 1):
                def get_new():
                    ...
                    if (p_name == "" and p_quantity == "" and p_recent == "" ):
                        ...
                        if (p_name != ""):
                            try:
                                ...
                                sql = "update instru set instru_name = %s where instru_id = %s"
                                val = [p_name,ids]
                        ...
                        if (p_quantity != ""):
                            try:
                                ...
                                sql = "update instru set quantity = %s where instru_id = %s"
                                val = [p_quantity, ids]
                        ...
                        if (p_recent != ""):
                            try:
                                ...
                                sql = "update instru set r_date = %s where instru_id = %s"
                                val = [p_recent, ids]
                ...
                id_Name = Label(home, text="Instrument Name", bg="turquoise1", fg="white", font=('Times', 14, 'bold'), padx=5,
    ...
    def delete():
        ...
        def gayab():
            ...
            if(flag):
                ...
                try:
                    cur = myconn.cursor()
                    sql = "DELETE FROM instru WHERE instru_id = %s"
